...
type DayType = 'WEEKDAYS' | 'SATURDAYS' | 'SUNDAYS & PUBLIC HOLIDAYS';

interface InvoiceGroupData {
  tier: DiscountTier;
  ...
  for (let d = 1; d <= totalDaysInMonth; d++) {
    ...
    if (!isIdle) {
      ...
      if (d === totalDaysInMonth) {
         periods.push({ start: currentStart, end: d, length: (d - currentStart + 1), tier: getDiscountTier(d - currentStart + 1) });
      }
    } else {
      if (currentStart !== null) {
        periods.push({ start: currentStart, end: d - 1, length: ((d - 1) - currentStart + 1), tier: getDiscountTier(((d - 1) - currentStart + 1)) });
        currentStart = null;
      }
    }
  }
...
const calculateRates = (baseRate: number | string): Rates => {
...
const PRESETS = [
  { name: 'Dropside', rate: 5200 },
  { name: 'ADT', rate: 6800 },
  { name: 'FEL', rate: 5485 },
  { name: 'Bulldozzer', rate: 7314 },
  { name: 'Skid Steer', rate: 2800 },
  { name: 'Concrete Cutter', rate: 478.26 },
];
...
const DayPicker: React.FC<DayPickerProps> = ({ month, selectedDays, onDaysChange }) => {
  const year = month.getFullYear();
  const monthIndex = month.getMonth();
  const lastDay = new Date(year, monthIndex + 1, 0);
...
const EquipmentCard: React.FC<EquipmentCardProps> = ({ item, currentMonth, onRemove, onUpdateIdleDays, onUpdateRates, onMonthChange }) => {
